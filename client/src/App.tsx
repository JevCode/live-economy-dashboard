...
function Ticker({ cur }: { cur: string }) {
  ...
  const items = [
    { l: "BRENT CRUDE", v: `$${vals.brent.toFixed(2)}`, c: "▼ −0.7% today", dn: true },
    { l: "WTI CRUDE",   v: `$${vals.wti.toFixed(2)}`,   c: "▼ −1.1% today", dn: true },
    { l: "GOLD XAU",    v: `$${vals.gold.toFixed(2)}/oz`, c: "▲ +0.50% today", dn: false },
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <KpiCard label="Brent Crude" value={fmt(LIVE.brentUSD, cur)} sub="per barrel · Apr 15 close" change="▼ −0.7% today" changeUp={false} accent="#f97316" />
        <KpiCard label="WTI Crude" value={fmt(LIVE.wtiUSD, cur)} sub="per barrel" change="▼ −1.1% today" changeUp={false} accent="#f97316" />
      ...
      <p className="text-[10px] text-white/20">Sources: Brent/WTI — Barchart / Pound Sterling Live / WSJ (Apr 15, 2026) · DXY — Investing.com · USD/MYR — Pound Sterling Live</p>
    </div>
  );
}
...
function GoldTab({ cur }: { cur: string }) {
  ...
  return (
    <div className="space-y-6 p-6">
      {/* KPIs */}
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        <KpiCard label="Gold Spot (24K/oz)" value={fmt(LIVE.goldOzUSD, cur)} sub="per troy ounce (31.1g)" change="▲ +0.50% today" changeUp={true} accent="#f0a500" />
