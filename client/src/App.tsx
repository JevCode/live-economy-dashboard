...
import { LIVE as LIVE_STATIC, KARATS, OIL_TIMELINE, GOLD_TIMELINE, COUNTRIES, STRIKE_EVENTS, WAR_STATS, NEWS, FX_RATES, type Country, type StrikeEvent, type NewsItem } from "./lib/data";

// ════ LIVE DATA HOOK ════
// Fetches real-time prices from open APIs; falls back to static LIVE data gracefully.
function useLiveData() {
  const [data, setData] = useState(LIVE_STATIC);
  ...
  useEffect(() => {
    async function fetchPrices() {
      setFetching(true);
      try {
        // Gold & FX via open.er-api.com (free, no key) + metals-api fallback
        // We use goldapi.io free tier for gold and exchangerate.host for FX
        const [fxRes, goldRes] = await Promise.allSettled([
          fetch("https://open.er-api.com/v6/latest/USD"),
          fetch("https://api.metals.live/v1/spot/gold"),
        ]);

        let newData = { ...LIVE_STATIC };

        if (fxRes.status === "fulfilled" && fxRes.value.ok) {
          const fx = await fxRes.value.json();
          if (fx?.rates?.MYR) newData.usdMyr = parseFloat(fx.rates.MYR.toFixed(4));
          if (fx?.rates?.AED) newData.usdAed = parseFloat(fx.rates.AED.toFixed(4));
        }

        if (goldRes.status === "fulfilled" && goldRes.value.ok) {
          const gold = await goldRes.value.json();
          const priceOz = gold?.[0]?.price ?? gold?.price ?? null;
          if (priceOz && priceOz > 1000) {
            newData.goldOzUSD = parseFloat(priceOz.toFixed(2));
            newData.goldGramUSD = parseFloat((priceOz / 31.1035).toFixed(2));
          }
        }

        setData(newData);
        setLastUpdated(new Date().toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Dubai" }) + " UAE");
      } catch {
        // silent fallback — static data stays intact
      } finally {
        setFetching(false);
      }
    }
  ...
  return { LIVE: data, lastUpdated, fetching };
}

// Re-export as LIVE for downstream components (will be overridden by hook in App)
...
type LiveData = typeof LIVE;

function fmt(usd: number, cur: string, liveDataOrDecimals?: LiveData | number, decimals = 2) {
  let ld: LiveData = LIVE;
  let dec = decimals;
  if (typeof liveDataOrDecimals === "number") { dec = liveDataOrDecimals; }
  else if (liveDataOrDecimals) { ld = liveDataOrDecimals; }
  const rate = cur === "MYR" ? ld.usdMyr : cur === "AED" ? ld.usdAed : 1;
  const val = usd * rate;
  ...
  return symbol + val.toLocaleString("en-US", { minimumFractionDigits: dec, maximumFractionDigits: dec });
}
...
function Ticker({ cur, liveData }: { cur: string; liveData: typeof LIVE }) {
  const [vals, setVals] = useState({
    brent: liveData.brentUSD, wti: liveData.wtiUSD, gold: liveData.goldOzUSD,
  });

  // Re-seed from live data when it updates
  useEffect(() => {
    setVals({ brent: liveData.brentUSD, wti: liveData.wtiUSD, gold: liveData.goldOzUSD });
  }, [liveData.brentUSD, liveData.wtiUSD, liveData.goldOzUSD]);
  ...
  const items = [
    ...
    { l: "USD/MYR",     v: `${liveData.usdMyr}`,         c: "Ringgit pressure", dn: true },
    { l: "USD/AED",     v: `${liveData.usdAed}`,         c: "Pegged stable", dn: false },
    { l: "DXY INDEX",   v: `${liveData.dxy}`,            c: "▼ −0.35%", dn: true },
    { l: "GOLD/g 24K",  v: fmt(liveData.goldGramUSD, cur, liveData),   c: `per gram`, dn: false },
    { l: "CRISIS DAY",  v: `Day ${liveData.crisisDay}`,  c: "Since Feb 28", dn: true },
    { l: "BRENT PEAK",  v: `$${liveData.brentPeak}`,     c: "Mar 18 peak", dn: true },
...
// ── OIL TAB ──
function OilTab({ cur, liveData }: { cur: string; liveData: LiveData }) {
  const pctPreWar = pct(liveData.brentUSD, liveData.brentPreWar);
  const pctFromPeak = pct(liveData.brentUSD, liveData.brentPeak);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        <KpiCard label="Iran Crisis — Day" value={`Day ${liveData.crisisDay}`} sub="Since Feb 28, 2026" change={`${pctPreWar} from $65 baseline`} changeUp={false} accent="#ef4444" />
        <KpiCard label="Brent Crude" value={fmt(liveData.brentUSD, cur, liveData)} sub={`per barrel · ${liveData.asOf} close`} change="▲ +3.7% today" changeUp={true} accent="#f97316" />
        <KpiCard label="WTI Crude" value={fmt(liveData.wtiUSD, cur, liveData)} sub="per barrel" change="▲ +4.4% today" changeUp={true} accent="#f97316" />
        <KpiCard label="Peak (Mar 18)" value={fmt(liveData.brentPeak, cur, liveData)} sub="Iran hit Qatar Ras Laffan" change={`${pctFromPeak} from peak`} changeUp={false} accent="#8b5cf6" />
        <KpiCard label="DXY USD Index" value={`${liveData.dxy}`} sub="Dollar basket" change="▼ −0.35% today" changeUp={false} accent="#3b82f6" />
        <KpiCard label="USD / MYR" value={liveData.usdMyr.toString()} sub="Mid-market rate" change="↑ Ringgit under pressure" changeUp={false} accent="#06b6d4" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[{ label: "BRENT CRUDE", usd: liveData.brentUSD }, { label: "WTI CRUDE", usd: liveData.wtiUSD }].map(oil => (
            ...
            <div className="grid grid-cols-3 gap-3">
              {["USD","MYR","AED"].map(c => (
                <div key={c} className="bg-white/3 rounded-lg p-3">
                  ...
                  <div className="text-base font-bold font-mono text-white mt-1">{fmt(oil.usd, c, liveData)}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-[#0d1117] border border-white/6 rounded-xl p-6">
        ...
        <div className="text-xs text-white/20 mb-5">Feb 27 – {liveData.asOf} · Key events overlaid</div>
        <ResponsiveContainer width="100%" height={240}>
          <AreaChart data={chartData} margin={{ top:10, right:10, left:0, bottom:0 }}>
            ...
            <ReferenceLine y={liveData.brentPreWar} stroke="#22c55e" strokeDasharray="4 4" strokeOpacity={0.5} label={{ value:"Pre-War $65", fill:"#22c55e", fontSize:9, fontFamily:"Space Mono" }} />
      ...
      <p className="text-[10px] text-white/20">Sources: Brent/WTI — Barchart / Pound Sterling Live / WSJ ({liveData.asOf} — OilPrice.com, TradingEconomics, Investing.com) · DXY — Investing.com · USD/MYR — open.er-api.com (live)</p>
    </div>
  );
}

// ── GOLD TAB ──
function GoldTab({ cur, liveData }: { cur: string; liveData: LiveData }) {
  ...
  return (
    <div className="space-y-6 p-6">
      {/* KPIs */}
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        <KpiCard label="Gold Spot (24K/oz)" value={fmt(liveData.goldOzUSD, cur, liveData)} sub="per troy ounce (31.1g)" change="▲ +1.91% today" changeUp={true} accent="#f0a500" />
        <KpiCard label="Gold Per Gram (24K)" value={fmt(liveData.goldGramUSD, cur, liveData)} sub="per gram" change="▲ +48% year-on-year" changeUp={true} accent="#f0a500" />
        <KpiCard label="All-Time High" value={`$${liveData.goldATH.toLocaleString()}`} sub={`${liveData.goldATHDate} (per oz)`} change="▼ −14.6% from ATH" changeUp={false} accent="#8b5cf6" />
        <KpiCard label="Gold in MYR (oz)" value={`RM ${(liveData.goldOzUSD * liveData.usdMyr).toLocaleString("en-MY", { maximumFractionDigits:0 })}`} sub="per troy ounce" change="▲ Safe-haven demand" changeUp={true} accent="#f0a500" />
        <KpiCard label="Gold in AED (oz)" value={`AED ${(liveData.goldOzUSD * liveData.usdAed).toLocaleString("en-US", { maximumFractionDigits:0 })}`} sub="per troy ounce" change="▲ AED peg stable" changeUp={true} accent="#f0a500" />
      ...
      {/* Karat cards grid */}
      <div>
        <div className="text-[10px] font-bold tracking-widest text-white/30 mb-4">GOLD BY KARAT GRADE — PER GRAM · USD / MYR / AED</div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {KARATS.map(k => {
            const priceG = liveData.goldGramUSD * k.purity;
            const priceOz = liveData.goldOzUSD * k.purity;
            return (
              <div key={k.k} className="bg-[#0d1117] border border-white/6 rounded-xl p-4 hover:border-white/10 transition-all group">
                ...
                <div className="text-xl font-bold font-mono" style={{ color: k.color }}>{fmt(priceG, "USD", liveData)}<span className="text-xs text-white/30 ml-1">/g</span></div>
                <div className="text-[10px] text-white/25 mt-1">
                  RM {(priceG * liveData.usdMyr).toFixed(2)}/g · AED {(priceG * liveData.usdAed).toFixed(2)}/g
                </div>
              ...
              {KARATS.map((k, i) => {
                ...
                return (
                  <tr key={k.k} className={`border-b border-white/4 hover:bg-white/2 transition-colors ${i % 2 === 0 ? "" : "bg-white/[0.01]"}`}>
                    ...
                    <td className="px-4 py-3 font-mono text-white/70">RM {(priceG * liveData.usdMyr).toFixed(2)}</td>
                    <td className="px-4 py-3 font-mono text-white/70">AED {(priceG * liveData.usdAed).toFixed(2)}</td>
                    <td className="px-4 py-3 font-mono text-white/40">${(liveData.goldOzUSD * k.purity).toLocaleString("en-US", { maximumFractionDigits:0 })}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <p className="text-[10px] text-white/20">Sources: Gold spot ${liveData.goldOzUSD.toLocaleString()}/oz — metals.live (live) · Gold MYR — goldpricez.com/my · ATH $5,602.22 — Jan 28, 2026</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="bg-[#0d1117] border-l-2 border-amber-500/60 border border-white/5 rounded-xl px-5 py-4 text-xs text-white/50 space-y-1">
        ...
        <div><span className="text-white/60 font-bold">As of: </span>Apr 22, 2026 · US-Iran talks stalled; Hormuz escort patrols ongoing</div>
      </div>
    ...
    <div className="space-y-5 p-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <div className="text-xs text-white/30 mt-0.5">Apr 22, 2026 · Crisis Day 53 · {NEWS.length} reports</div>
        </div>
...
// ── CURRENCIES TAB ──
function CurrenciesTab({ liveData }: { liveData: LiveData }) {
  const [fromCur, setFromCur] = useState("USD");
  const [amount, setAmount] = useState("100");

  function convert(a: number, from: string, to: string) {
    const toUSD = from === "USD" ? a : from === "MYR" ? a / liveData.usdMyr : from === "AED" ? a / liveData.usdAed : a;
    if (to === "USD") return toUSD;
    if (to === "MYR") return toUSD * liveData.usdMyr;
    if (to === "AED") return toUSD * liveData.usdAed;
    return toUSD;
  }
    ...
    <div className="space-y-6 p-6">
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-3">
            {["USD","MYR","AED"].map(to => {
              ...
              const rate = to === "USD" ? 1 : to === "MYR" ? liveData.usdMyr : liveData.usdAed;
      ...
      <div className="bg-[#0d1117] border border-white/6 rounded-xl overflow-hidden">
        <div className="p-4 border-b border-white/5">
          <div className="text-[10px] font-bold tracking-widest text-white/30">FX RATES — APR 22, 2026 MID-MARKET</div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
      ...
      <div className="bg-[#0d1117] border-l-2 border-blue-500/60 border border-white/5 rounded-xl px-5 py-4 text-xs text-white/50 leading-relaxed">
        <span className="text-white/70 font-bold">DXY US Dollar Index: {liveData.dxy} (▼ −0.46% today)</span> — Dollar softening as Hormuz partial reopening eases supply fears. EUR/USD at 1.089. The DXY measures USD against a basket of 6 currencies (EUR, JPY, GBP, CAD, SEK, CHF).
      </div>
...
export default function App() {
  ...
  const { LIVE: livePrices, lastUpdated, fetching } = useLiveData();

  // Propagate live LIVE to all child tabs by passing as prop
  const liveCtx = livePrices;

  return (
    <div className="min-h-screen bg-[#070a0f] text-white flex flex-col">
      <Ticker cur="USD" liveData={liveCtx} />
      ...
      <header className="sticky top-9 z-40 bg-[#070a0f]/95 backdrop-blur border-b border-white/5 h-14 flex items-center justify-between px-5 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:block text-[10px] font-mono text-white/20">APR 22, 2026</div>
          {/* Live indicator */}
          {lastUpdated && (
            <div className="hidden sm:flex items-center gap-1.5 text-[9px] font-mono text-emerald-400/60">
              <span className={`w-1.5 h-1.5 rounded-full inline-block ${fetching ? "bg-amber-400 animate-pulse" : "bg-emerald-500"}`} />
              {fetching ? "UPDATING…" : `LIVE · ${lastUpdated}`}
            </div>
          )}
      ...
      <main className="flex-1">
        {tab === "oil"        && <OilTab cur="USD" liveData={liveCtx} />}
        {tab === "gold"       && <GoldTab cur="USD" liveData={liveCtx} />}
        {tab === "map"        && <WorldMapTab />}
        {tab === "war"        && <WarTab />}
        {tab === "news"       && <NewsTab />}
        {tab === "currencies" && <CurrenciesTab liveData={liveCtx} />}
      </main>
      ...
      <footer className="border-t border-white/4 px-6 py-4 flex flex-wrap justify-between gap-2 text-[10px] text-white/20 font-mono">
        <span>Jeff's MarketIntel v3 · Apr 22, 2026 · Crisis Day {liveCtx.crisisDay} · <a href="https://github.com/JevCode/live-economy-dashboard" className="hover:text-amber-400 transition-colors">GitHub</a></span>
        <span>Data: open.er-api.com · metals.live · Barchart · Pound Sterling Live · Investing.com · Wikipedia</span>
        <span>Live prices refresh every 5 min · 08:00 UAE daily reset</span>
      </footer>
    </div>
  );
}
